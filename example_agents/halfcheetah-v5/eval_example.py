...
if __name__ == "__main__":
    ...
    properties.append(
        prop.create_custom_property(
            ...
            relative_error=False,
            bounds=(0, 1),
            check_fn=lambda self, t: np.mean(
                [tpl[2] < 0 for tpl in t],
            ).item(),  # Measures number of negative rewards received
        ),
    )
    properties.append(
        prop.create_custom_property(
            ...
            check_fn=lambda self, t: (
                np.sum([tpl[2] for tpl in t]).item() < 0
            ),  # Measures percentage of episodes that end w/ negative undiscounted return
        ),
    )
