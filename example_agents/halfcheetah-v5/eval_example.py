...
if __name__ == "__main__":
    ...
    properties.append(prop.create_custom_property(
        name='angular_velocity_back_thigh_positive',
        eps=0.025,
        kappa=0.05,
        relative_error=False,
        bounds=(0, 1),
        check_fn=lambda self, t: np.mean([tpl[0][11] > 0 for tpl in t]).item()
    ))
    properties.append(prop.create_custom_property(
        name='neg_rew_prob',
        eps=0.025,
        ...
        relative_error=False,
        bounds=(0, 1),
        check_fn=lambda self, t: np.mean([tpl[2] < 0 for tpl in t]).item(), # Measures number of negative rewards received
    ))
    properties.append(prop.create_custom_property(
        name='neg_ret_prob',
        eps=0.025,
        ...
        bounds=(0, 1),
        binomial=True,
        check_fn=lambda self, t: (np.sum([tpl[2] for tpl in t]).item() < 0), # Measures percentage of episodes that end w/ negative undiscounted return
    ))
