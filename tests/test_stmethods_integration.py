...
def setup_module():
    ...
    for property in properties.values():
        assert (property.epsilon is None and "fixed" in property.name) or (
            property.epsilon is not None and "fixed" not in property.name
...
def assert_convergence(
    ...
    assert v_total_episodes is None or math.isclose(
        property.num_episodes,
        v_total_episodes,
    ...
    assert v_intv is None or (
        math.isclose(intv[0], v_intv[0], rel_tol=REL_TOLERANCE)
        and math.isclose(intv[1], v_intv[1], rel_tol=REL_TOLERANCE)
    )

    intv_size = intv[1] - intv[0]
    assert intv_size >= 0, (
        f"Upper interval bound should be greater than lower bound; Interval: ({float(intv[0]):3f},{float(intv[1]):3f})"
    )
    if property.epsilon is not None:
        epsilon = property.epsilon * (property.mean if property.relative_error else 1)
        assert intv_size <= 2 * epsilon, (
            f"Interval is too large; len(({float(intv[0]):3f},{float(intv[1]):3f})) = {intv_size:3f} > {2 * epsilon:3f}"
        )


def assert_statistics_selection(property, st_method_cls):
    assert type(property.st_method) is st_method_cls, (
...
@pytest.mark.parametrize(
    ...
    [
        # Binomial Tests
        ("binomial_abs_sound", ClopperPearsonIntervalMethod, None, None, None, None, None),
        (
            "binomial_rel_sound",
            EBStopMethod,
            1700,
            ...
            0.13678754325259496,
            0.36984800020088654,
            (0.6835047683613917, 1.0252571525420875),
        ),
        (
            "binomial_abs_unsound",
            NormalIntervalMethod,
            1700,
            0.836470588235294,
            0.13678754325259496,
            0.36984800020088654,
            (0.8261482740776852, 0.8467929023929027),
        ),
        (
            "binomial_rel_unsound",
            NormalIntervalMethod,
            1700,
            0.836470588235294,
            0.13678754325259496,
        ...
        (
            ...
            1443.2400932417527,
            37.98999991105229,
            (66.98752776044076, 76.98752776044076),
        ),
        (
            "bounded_rel_sound",
            EBStopMethod,
            ...
            1443.2400932417527,
            37.98999991105229,
            (58.16942104253408, 87.25413156380111),
        ),
        (
            "bounded_abs_unsound",
            StudentsTMethod,
            ...
            1443.2400932417527,
            37.98999991105229,
            (70.92724160581396, 73.04781391506755),
        ),
        (
            "bounded_rel_unsound",
            StudentsTMethod,
        ...
        (
            ...
            349.72988096885786,
            20.331898157795365,
            (30.644897553851735, 31.77980832850119),
        ),
        (
            "unbounded_rel",
            StudentsTMethod,
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("fixed_binomial_sound", ClopperPearsonIntervalMethod, None, None, None, None, None),
        (
            "fixed_binomial_unsound",
            NormalIntervalMethod,
            2000,
            0.8495,
            0.12784975,
            0.3575608339849318,
            (0.8400932839491071, 0.8589937026426394),
        ),
        # Fixed Bounded Tests
        (
            "fixed_bounded_sound",
            ...
            1184.4999046689964,
            34.41656439374791,
            (69.80995029922501, 77.56629654990643),
        ),
        (
            "fixed_bounded_unsound",
            StudentsTMethod,
            2000,
            74.12348035016905,
            1184.4999046689964,
            34.41656439374791,
            (73.23793973076012, 75.00902096957797),
        ),
        ...
        (
            ...
            365.391096,
            20.391607489357,
            (31.163322506910344, 32.21267749308958),
        ),
    ],
)
