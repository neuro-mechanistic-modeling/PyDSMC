...
def test_no_agent_predict(setup_module):
    ...
    with pytest.raises(TypeError, match="No callable predict function or agent given"):
        evaluator.eval(
            agent=None,
            episode_limit=100,
            save_every_n_episodes=100,
...
def test_not_enough_envs(setup_module):
    ...
    with pytest.raises(
        ValueError,
        match="Number of environments must be at least the same as number of threads",
    ):
        evaluator.eval(
            agent=agent,
            episode_limit=100,
...
def test_policy_runs_per_iter(setup_module):
    ...
    with pytest.raises(
        ValueError,
        match="Number of initial episodes, and per policy run, must be at least 1",
    ):
        evaluator.eval(
            agent=agent,
            episode_limit=100,
